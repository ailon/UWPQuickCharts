    ...
    public class PieChart : Control
    {
        ...
        private void AddSlices()
        {
            for (int i = _slices.Count; i < _values.Count; i++)
            {
                ...
                _slices.Add(slice);
                slice.PointerEntered += Slice_PointerEntered;
                slice.PointerMoved += Slice_PointerMoved;
                slice.PointerExited += Slice_PointerExited;
                AddSliceToCanvas(slice);
            }
        }


        private void Slice_PointerEntered(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e)
        {
            ShowBalloonAtLocation(sender, e);
        }

        private void ShowBalloonAtLocation(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e)
        {
            GeneralTransform gt = (sender as Slice).TransformToVisual(_sliceCanvasDecorator);
            DisplayBalloon(sender as Slice, gt.TransformPoint(e.GetCurrentPoint(sender as UIElement).Position), e.Pointer.PointerDeviceType);
        }

        private void Slice_PointerMoved(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e)
        {
            ShowBalloonAtLocation(sender, e);
        }

        private void Slice_PointerExited(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e)
        {
            HideBaloon();
        }
